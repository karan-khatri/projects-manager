...
const LoginFormPage = () => {
  const { loginUser, user, token, loading } = useGlobalContext();
  ...
  return (
    <Container maxWidth={false} sx={{ width: '100%', height: 'calc(100vh - 80px)', backgroundImage: `url()`, backgroundColor: '#eee' }}>
      <Box display={'flex'} justifyContent={'center'} flexDirection='column' alignItems='center' width={'100%'} height={'100%'}>
        ...
        <Typography paragraph align='center'>
          Not already a user? <Link to='/auth/register'>Sign Up</Link>
        </Typography>
      </Box>
    </Container>
  );
